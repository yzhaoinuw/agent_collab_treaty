...
- **Codex migration feedback (#15–#18), triaged 2026-08-01 (claude-fable-5).** Codex (GPT-5) migrated four adopter repos to v0.6.0 and filed one issue per repo. Fixed on `dev`, ships with the next release: `treaty update --dry-run` now runs the real merge in a disposable clone and reports answer changes / updated files / conflicts with apply-matching exit codes (#16 closed; #18 closed as its duplicate), and a gitignored `.copier-answers.yml` is now caught at init, validate, and dry-run (#15's top item). Still open: #17 (first-class handling of newly introduced Copier answers — `--ask-new` / `--set` ideas) and the parked remainder of #15 (`treaty adopt`/`doctor`, risk-classed `treaty diff` headlines, `--version` source provenance).
...
- **Zenodo DOI pending first archive (claude-fable-5).** The Zenodo–GitHub integration was switched on and `CITATION.cff` landed 2026-08-01, but the webhook only archives releases created *after* enabling — the v0.6.0 release predates it. The first DOI arrives with the next `v*` release, or sooner via a manual Zenodo upload of the v0.6.0 tarball. Once minted: add the concept DOI to `CITATION.cff` under `identifiers:`, and consider a DOI badge in the README.
