...
- **`docs_dir` / nested docs layout — on branch `nested-docs`, not merged (claude-opus-5, 2026-08-12).** Working docs install under `treaty_docs/`; `AGENTS.md` and `project_overview.md` stay at the root. Pre-v0.8.0 adopters are pinned to the flat layout automatically and verified unaffected (14/14 real adopters: identical conflicts and touched files vs. a v0.7.0 control). **Windows pass is complete** (Codex/GPT-5, PR #20): 12 local adopters swept branch-vs-control with identical results, plus nested case-collision handling, nested `treaty diff`, and the full suite with `PYTHONUTF8` absent. Two findings came out of it and are both resolved — multi-segment `docs_dir` (`2766815`) and the Unicode-arrow crash (`fae916c`, on `dev`). **Nothing technical now blocks the merge; the two remaining gates are maintainer calls: merging #20, and the version number** — the flat-rendering baseline in `tests/test_docs_dir.py` is pinned to `v0.7.0` and assumes v0.8.0 is the release that introduces this.
- **`treaty init --source .` records a source that cannot be resolved later (pre-existing, not in PR #20) (claude-opus-5, 2026-08-12).** Copier records `--source` verbatim, so a relative `.` lands in `.copier-answers.yml` as `_src_path: .`, which afterwards resolves to *the adopter's own repo*. Every later `treaty diff` / `treaty update` then tries to `git checkout <ref>` inside the wrong repository and dies. Installing from an untagged commit compounds it: `_commit` is recorded as a `git describe` string (`v0.7.0-1-gfae916c`) that is not a resolvable ref. Confirmed identical on `dev`, so it predates the layout branch. Released adopters are unaffected — they install from `gh:…` at a real tag. But `AGENTS.md` tells maintainers to smoke-test with exactly `--source . --ref HEAD`, and two different agents (Claude, then Codex on the #20 Windows sweep) each lost time to it in one week. Likely fix: in `init`, resolve `--source` to an absolute path when it names an existing local directory, before handing it to Copier. The `_commit` describe-string half is Copier's own recording and is less fixable on our side; a warning when the recorded `_commit` is not a resolvable ref would at least make it self-diagnosing.
