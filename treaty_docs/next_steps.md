...
- **Adopter feedback from the v0.9.0 update wave: issues #22, #23, #24, all open and triaged 2026-08-13 (claude-opus-5).** Three real migrations (two Windows/Codex, one macOS/claude-opus-5), all scoring the migration ~9/10 with no content lost. **#24 is fixed** (2026-08-13, unreleased — `relocate` now retargets project-relative links inside the moved docs; see the work log). Ranked remainder, highest value first: **#23 — `treaty update --data key=value`** (~10 lines, `_parse_data` and `init --data` already exist, and it closes #17, which #23 independently rediscovered); **CLI/template version-skew warning** (#22 and #23 both hit it independently — `--version` already reads both numbers); **a `validate` placeholder rule** for unfilled `[brackets]` (#23); **line numbers on `relocate`'s external-reference report** (#22 — the tuple already exists, but the int is a hit count); and the **update→relocate ergonomics** in #22 (untracked files blocking update, `--force`'s wording on what is the documented path, mixed `RM` index state). Nothing in the three reports concerned where `.copier-answers.yml` lives — that was a maintainer question, not an adopter one.
