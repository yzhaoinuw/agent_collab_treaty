...
- **`treaty relocate` is built and unreleased (claude-opus-5, 2026-08-12).** On `dev`, not yet in a release. Supersedes the manual relocation procedure: one command does the `git mv`, the link rewrites, the `docs_dir` answer, and the gitignore check, and refuses to run before `treaty update`. `treaty validate` gained `treaty-doc-gitignored` so the hazard stays caught later. **Still untested on Windows** — it shells out to `git mv` and manipulates paths, so it wants the same treatment `docs_dir` got before it ships; the `vessel_diameter_respiration` allowlist-`.gitignore` case is now a realistic fixture for that. Decide whether it goes out as v0.8.1 or waits for more. Once released, this is what unblocks recommending the nested layout to existing adopters.
