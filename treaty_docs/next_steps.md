...
- **Adopter feedback from the v0.9.0 update wave: issues #22, #23, #24, all open and triaged 2026-08-13 (claude-opus-5).** Three real migrations (two Windows/Codex, one macOS/claude-opus-5), all scoring the migration ~9/10 with no content lost. **#24 is fixed and closed** (2026-08-13, unreleased — `relocate` now retargets project-relative links inside the moved docs). **#23's `--data` flag is done too** (2026-08-13, unreleased — also closes **#17**; `--data docs_dir=…` is deliberately refused and points at `treaty relocate`). #23's other three items are still open: the first-run `work_log.md` boilerplate (#23.2, now unblocked by the freeze decision), placeholder sections arriving on update (#23.3), and the version-skew warning. Ranked remainder, highest value first: **CLI/template version-skew warning** (#22 and #23 both hit it independently — `--version` already reads both numbers); **a `validate` placeholder rule** for unfilled `[brackets]` (#23); **line numbers on `relocate`'s external-reference report** (#22 — the tuple already exists, but the int is a hit count); and the **update→relocate ergonomics** in #22 (untracked files blocking update, `--force`'s wording on what is the documented path, mixed `RM` index state). Nothing in the three reports concerned where `.copier-answers.yml` lives — that was a maintainer question, not an adopter one.
- **The flat-rendering freeze is settled: paths frozen, content free (2026-08-13, claude-opus-5).** The blanket byte-identity assertion is gone, replaced by a path-set freeze plus behavioral merge tests. See the work-log entry for the measurements and the three-tier policy now in `AGENTS.md`. This unblocked #19, #23.2, and #23.3a.
