...
- **`treaty relocate` is built and unreleased (claude-opus-5, 2026-08-12).** On `dev`, not yet in a release. Supersedes the manual relocation procedure: one command does the `git mv`, the link rewrites, the `docs_dir` answer, and the gitignore check, and refuses to run before `treaty update`. `treaty validate` gained `treaty-doc-gitignored` so the hazard stays caught later. **Windows-verified** (Codex/GPT-5, issue #21): all seven checks passed on Git 2.42 / `core.autocrlf=true`, including `git mv` renames, the ordering guard, and the deny-all `.gitignore` sequence. One finding fixed — a case-only target now reports a single clear blocker instead of four spurious collisions. Nothing technical is outstanding; the only open question is whether it ships as v0.8.1 or waits for more to accumulate. Once released, this is what unblocks recommending the nested layout to existing adopters.
