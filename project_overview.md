...
- Requires Copier 9.0+.
- Defines the current template questions: `integration_branch`, `env_activation`, and `test_command`.
...
- Whether per-agent pointer files should be generated by default, opt-in by multiselect, or left as local-only manual setup.
